...
function start_act1() {
    ...
    <div class='collapse divide' style='style='margin-top: 2vw; 'width: 80%; margin: auto;' id='tb1-box'>
		...
		<h4>Given that </h4>
		<div style='text-align: center;'>$$ f(x) = \\frac{x^2}{${a}} - ${b}sinx  $$</div>
    <br>
...
function calculate_data() {
    ...
    for (let i = 0; i < 20; i++) {
        ...
        f_x1 = Number((Math.pow(x1, 2) / a - b * Math.sin(x1)).toFixed(3));
        f_x2 = Number((Math.pow(x2, 2) / a - b * Math.sin(x2)).toFixed(3));
        if (f_x1 < f_x2) {
            x_l = x2;
        }
        else {
